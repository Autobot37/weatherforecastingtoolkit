...
# (Bottleneck, EncBlock, and DecBlock classes are unchanged)
# ...

# ------------------------------------------------------------
# Full AE with Transformer Bottleneck -> Flattened Output
# ------------------------------------------------------------
class PosAwareAutoEncoder(nn.Module):
    def __init__(
        ...
        latent_dim_flat = latent_channels * self.latent_h * self.latent_w # 4096
        ...
        self.pos_emb = nn.Parameter(torch.randn(1, self.latent_h * self.latent_w, latent_channels))
        transformer_layer = nn.TransformerEncoderLayer(
            d_model=latent_channels, nhead=8, dim_feedforward=2048, dropout=0.1, batch_first=True
        )
        self.transformer = nn.TransformerEncoder(transformer_layer, num_layers=6)

        # This layer learns to un-flatten the vector for the decoder
        self.from_timeseries = nn.Linear(latent_dim_flat, latent_dim_flat)
    ...
    def encode(self, x):
        z_spatial = self.enc(x)
        z_seq = z_spatial.flatten(2).permute(0, 2, 1)
        z_seq = z_seq + self.pos_emb
        z_processed_seq = self.transformer(z_seq)
        
        # Flatten the final sequence to get the desired (B, 4096) shape
        z_flat = z_processed_seq.flatten(1)
        return z_flat

    def decode(self, z_flat):
        # Use the linear layer to prepare the vector for reshaping
        z_unflattened = self.from_timeseries(z_flat)
        
        # Reshape back to a spatial grid for the convolutional decoder
        z_spatial = z_unflattened.view(
            -1, self.latent_channels, self.latent_h, self.latent_w
        )
        
        return self.act(self.dec(z_spatial))

    def forward(self, x):
        z_flat = self.encode(x)
        recon = self.decode(z_flat)
        return recon, z_flat
...
if __name__ == "__main__":
    ...
    # The latent shape is now the desired (B, 4096)
    print("latent:", z.shape) # Will be torch.Size([2, 4096])
