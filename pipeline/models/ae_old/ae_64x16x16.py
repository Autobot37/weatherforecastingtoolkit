...
class Encoder(nn.Module):
    def __init__(self, in_channels=1, base_channels=256):
        ...
        self.final = nn.Conv2d(base_channels * 8, 64, 1)  # Compress to 64 channels
...
class Decoder(nn.Module):
    def __init__(self, out_channels=1, base_channels=256):
        super().__init__()
        self.proj = nn.Conv2d(64, base_channels * 8, 1)
...
class AutoEncoder(nn.Module):
    def __init__(self):
        super().__init__()
        self.encoder = Encoder(base_channels=128)
        self.decoder = Decoder(base_channels=128)
