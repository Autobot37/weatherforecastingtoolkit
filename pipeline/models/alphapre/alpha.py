...
def get_model(cfg):
    model = AlphaPre(
        pre_seq_length=cfg.T_in,
        aft_seq_length=cfg.T_out,
        input_shape=cfg.input_shape,
        input_dim=cfg.img_channels,
        hidden_dim=cfg.dim,
        n_layers=cfg.n_layers,
        spec_num=getattr(cfg, 'spec_num', 20),
        pha_weight=getattr(cfg, 'pha_weight', 0.01),
        anet_weight=getattr(cfg, 'anet_weight', 0.1),
        amp_weight=getattr(cfg, 'amp_weight', 0.01),
        aweight_stop_steps=getattr(cfg, 'aweight_stop_steps', 10000),
    )
    return model
